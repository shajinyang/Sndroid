...
import com.sjy.sndroid.cache.callback.CacheCallback;
...
public final class CacheHelper {
    ...
    private volatile static CacheHelper mCacheManager;

    public static CacheHelper getInstance() {
        if (mCacheManager == null) {
            synchronized (CacheHelper.class) {
                if (mCacheManager == null) {
                    mCacheManager = new CacheHelper();
                }
            }
        }
    ...
    private CacheHelper() {
        File diskCacheDir = getDiskCacheDir(application.getApplicationContext(), CACHE_DIR);
        ...
        if (diskCacheDir.getUsableSpace() > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = DiskLruCache.open(diskCacheDir,
                        getAppVersion(application.getApplicationContext()), 1/*一个key对应多少个文件*/, DISK_CACHE_SIZE);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
     ...
     * 异步设置缓存
     */
    public void setSynCache(final String key, final String value) {
        new Thread() {
            @Override
            public void run() {
                setCache(key, value);
     ...
     * 异步获取缓存
     */
    public void getSynCache(final String key, final CacheCallback callback) {
        new Thread() {
            @Override
            public void run() {
